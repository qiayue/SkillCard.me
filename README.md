...
| AI 能力 | Workers AI (@cf/meta/llama-3.1-8b-instruct) | Skill 自动分类、描述生成、README 摘要提取、兼容性判断 |
| 定时任务 | Workers Cron Triggers | 每天自动执行数据采集、AI 处理和评分计算 |
...
CREATE TABLE skill_sources (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  skill_id INTEGER NOT NULL,
  source_type TEXT NOT NULL,              -- 'github_search' / 'awesome_list' / 'manual' / 'social_media'
...
4. 调用 Workers AI 处理（仅对新入库或 README 有更新的 Skill）：
...
5. 写入 daily_snapshots 表
6. 触发评分计算，将 overall_score 同步写入 daily_snapshots
7. 刷新 KV 缓存
```
...
**渠道二：Awesome-list 仓库解析（自动）**

```
1. 维护一份已知的 Skill 收集仓库列表（如 awesome-claude-code-skills 等）
2. 定期拉取这些仓库的 README/列表文件
3. 解析其中的 GitHub 链接，提取 Skill 仓库 URL
4. 按渠道一相同流程入库和生成内容
...
2. Worker 自动拉取 GitHub 信息 + AI 生成 Wiki 内容
...
```
每天 UTC 02:00  ─── Cron Worker: 数据采集 + AI 处理 ───┐
                    (GitHub 数据 → AI 分类/摘要 → 入库)   │
                                                          │
...
4. 对所有入库 Skill 批量运行 AI 生成 Wiki 内容
5. 触发首次 Pages 构建上线
```
